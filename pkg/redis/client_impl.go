...
func (client *ClientImpl) GetBoughtProductsCache() ([]models.BoughtProductsQuantity, error) {
...
func (client *ClientImpl) GetBoughtItemsCache() ([]models.BoughtItemsQuantity, error) {
...
func (client *ClientImpl) SetBoughtProductsCache(boughtProducts []models.BoughtProductsQuantity, expiresAfter time.Duration) error {
	var input []string
...
func (client *ClientImpl) SetBoughtItemsCache(boughtItems []models.BoughtItemsQuantity, expiresAfter time.Duration) error {
	var input []string
...
func (client *ClientImpl) PutRequestToQueue(request string) error {
	return client.rds.RPush(client.ctx, "requests", request).Err()
}

func (client *ClientImpl) PopRequestFromQueue() (string, error) {
...
func (client *ClientImpl) PublishResult(topic, message string) error {
	return client.rds.Publish(client.ctx, topic, message).Err()
}

func (client *ClientImpl) SubscribeForResult(topic string, timeout int) (string, error) {
...
func (client *ClientImpl) GetLockCounter(token string) (int, error) {
...
func (client *ClientImpl) Lock(token string) error {
	return client.rds.IncrBy(client.ctx, "lock:"+token, 1).Err()
}

func (client *ClientImpl) Unlock(token string) error {
	return client.rds.DecrBy(client.ctx, "lock:"+token, 1).Err()
}

func (client *ClientImpl) GetRetryCount(token string) (int, error) {
...
func NewRedisClient(ctx context.Context, log *log.Logger, config *config.AppConfig) *ClientImpl {
